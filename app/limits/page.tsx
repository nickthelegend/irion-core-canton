...
export default function LimitsPage() {
  ...
  const refresh = useCallback(async () => {
    if (!address) return
    setProfile(await irion.getProfile(address).catch(() => null))
  }, [address])
    ...
    <ConnectGate>
      <div className="flex-1 flex flex-col py-8 gap-8 w-full font-mono text-white max-w-5xl mx-auto">
        <div className="flex flex-col gap-1">
          <span className="font-mono text-[10px] tracking-[0.4em] text-primary/60 uppercase">IRION // Credit_Limits · stellar_{NETWORK}</span>
          ...
          <div className="glass-card rounded-lg border border-primary/30 overflow-hidden shadow-2xl col-span-1 md:col-span-2 relative group">
            ...
            <div className="p-8 flex flex-col gap-2">
              ...
              <div className="flex items-baseline gap-3">
                <span className="text-5xl md:text-6xl font-black tracking-tighter text-white">{profile ? available.toLocaleString() : "—"}</span>
              ...
              <p className="text-[10px] text-white/30 max-w-md mt-4 leading-relaxed">
                Your limit grows as you repay on time. It is applied to your on-chain CreditProfile after a
                zero-knowledge score proof — no manual increase requests needed.
              </p>
            </div>
          </div>

          <div className="glass-card rounded-lg border border-white/10 overflow-hidden shadow-2xl flex flex-col">
            <div className="bg-white/5 px-4 py-2 border-b border-white/10 flex justify-between"><span className="text-[10px] text-white/40 uppercase tracking-widest">ZK_Score</span><Lock className="w-3 h-3 text-purple-400" /></div>
        ...
        {!profile && (
          <div className="bg-[#0d0f14] border border-border/30 rounded-3xl p-8 flex flex-col items-center text-center gap-4">
            ...
            <p className="text-sm text-foreground/50 max-w-sm">No credit profile detected. Open one in the money market to start building your limit and ZK score.</p>
          </div>
        )}
          ...
          <Link href="/lend-borrow" className="bg-primary/5 border border-primary/20 rounded-3xl p-6 flex items-center justify-between hover:bg-primary/10 transition-colors group">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-foreground/50 mt-1">Borrow against collateral or your ZK score.</p>
              </div>
            </div>
